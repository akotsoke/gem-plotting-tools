...
def fline(x,par):
	if(x[0]>minimum and x[0]<minimum+4):
		TF1.RejectPoint()
		return 0;
...
for vfat in range(0,24):
    ...
    #fit_f_Peak[vfat]= r.TF1("fit%i_f_peak"%vfat,"[0]",minimum,minimum+4)  #Fit function for the peak range
    #fit_f_Peak[vfat].SetParameter(0, 0.03)
    print "vfat is="+str(vfat)
    ...
    for minimum in range(20,40):

        #print minimum 
        #fit_f_Peak1[vfat]= r.TF1("fit%i_f_peak1"%vfat,"[0]",10,20)
        fit_f_Peak[vfat]= r.TF1("fit%i_f_peak"%vfat,"[0]",minimum,minimum+4)
        fit_f_Peak[vfat].SetParameter(0, 0.03)
        lat_ga[vfat].Fit(fit_f_Peak[vfat],"RQ")
        par_com=fit_f_Peak[vfat].GetParameters()
        #print par_com[0]

        if minimum==20:
            MaxParValue= par_com[0]
            ErrYPeak=fit_f_Peak[vfat].GetParError(0)
            #print MaxParValue
        elif par_com[0]>MaxParValue:
            MaxParValue=par_com[0]
            ErrYPeak=fit_f_Peak[vfat].GetParError(0)
        #    print "final value="
        #    print MaxParValue
        ...
        pass
    #print MaxParValue    
    par_Peak.append(MaxParValue)
    errY_Peak.append(fit_f_Peak[vfat].GetParError(0))
...
tgr.GetYaxis().SetTitleOffset(1.4)
...
leg= r.TLegend()
leg.AddEntry(tgr,"Fit Signal+Background","lep")
leg.AddEntry(tgr_NoPeaks,"Fit Background","lep")
leg.SetBorderSize(0)
leg.SetFillStyle(0)
leg.SetX1NDC(0.18)
leg.SetX2NDC(0.60)
...
#tgr.GetYaxis().SetRangeUser(0,0.01)
...
par_fraction = np.array((par_Peak-par_NoPeaks)/par_NoPeaks)
...
tgr_fraction.GetYaxis().SetRangeUser(0,10)
tgr_fraction.GetXaxis().SetTitle("VFAT Position")
tgr_fraction.GetYaxis().SetTitle("Average_{InPeak}-Average_{OutPeak}/Average_{OutPeak}")
